 ...
 * @param max_bytes: the maximum number of bytes to check for number of
 *                   characters
 * @return: the length of the string up to max_bytes, or max_bytes if no null
 *          terminator is found
 ...
 * @return: a newly allocated string that is a duplicate of the input string,
 *          or NULL on failure
 ...
 * @return: a pointer to the buffer containing the input string, or NULL if an
 *          error occurs
 ...
 * characters discarded when the input received by the most recently executed
 * mutils_getsn() exceeds the provided buffer size.
 *
 * This count is reset every time mutils_getsn() is called and remains 0 if
 * the input does not exceed the buffer size.
 *
 * Since this variable is not atomic, it must not be accessed while
 * mutils_getsn() is in progress.
 *
 * In environments where THREAD_LOCAL is defined, this variable is
 * thread-safe.
 */
#ifdef THREAD_LOCAL
	extern THREAD_LOCAL size_t getsn_truncated_len;
#else
	extern size_t getsn_truncated_len;
#endif

/*
 * bool_text
 * @param flag: a boolean value (true or false)
 * @return: a pointer to a static string representing the boolean value,
 ...
 * @return: next power of two greater than or equal to n, returns 0 if the
 *          result would overflow size_t
 */
